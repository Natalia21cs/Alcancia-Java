...
public class Alcancia {
   ...
   public int calcularTotalDinero() {
    ...
    public int obtenerDenominacionMasNumerosa() {
        int[] denominaciones = {numeroMonedas50, numeroMonedas100, numeroMonedas200,
                                numeroMonedas500, numeroMonedas1000};
        int maxDenominacion = 0;
        int maxCantidad = 0;

        for (int i = 0; i < denominaciones.length; i++) {
            if (denominaciones[i] > maxCantidad) {
                maxCantidad = denominaciones[i];
                maxDenominacion = (i + 1) * 50; // Calcula la denominación en base al índice
            }
        }
